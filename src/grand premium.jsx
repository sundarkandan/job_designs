  ...
  useEffect(() => {
    ...
    const handleImageLoad = () => {
      ...
      if (loadedCount === totalImages) {
        setIsLoading(false);
        // மென்மையான fade-out அனிமேஷனுக்காக சிறிய தாமதம்
    ...
    imageUrls.forEach(url => {
      ...
      img.onerror = handleImageLoad; // எரர் வந்தாலும் லோடிங் ஸ்கிரீன் மறைய வேண்டும்
    });
  }, []);
              ...
              <Reveal key={i} delay={i * 100}>
                <div className={`card-hover rounded-2xl overflow-hidden border group ${D ? 'bg-zinc-900 border-zinc-800 hover:border-yellow-400/40' : 'bg-white border-amber-200 hover:border-yellow-500/40'}`}>
                  <div className="relative h-44 overflow-hidden bg-zinc-800">
                    ...
                    <div className="absolute top-3 right-3 bg-yellow-400 text-zinc-950 text-xs font-black px-2.5 py-1 rounded-lg uppercase shadow">{v.tag}</div>
                  </div>
                  <div className="p-5">
