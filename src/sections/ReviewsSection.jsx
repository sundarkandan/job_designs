...
export default function ReviewsSection({ dark, t }) {
  ...
  return (
    <section id="reviews" className={`py-20 border-t transition-colors duration-300 ${D ? 'border-zinc-900/40 bg-zinc-950/10' : 'border-zinc-200 bg-zinc-100/60'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {/* Map through the dynamic t.reviewsData */}
          {t.reviewsData.map((rev, idx) => (
            <GlassReveal key={idx} delay={idx * 100}>
              <div className="liquid-hover glass-panel p-6 rounded-2xl space-y-4 flex flex-col justify-between h-full shadow-lg">
