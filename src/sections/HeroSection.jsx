...
export default function HeroSection({
  ...
  // ── ✅ FIXED: Refs declared INSIDE the component ──
  const pickupRef = useRef(null);
  const dropRef   = useRef(null);

  // ── ✅ FIXED: Hooks called INSIDE the component ──
  usePlacesAutocomplete(pickupRef, (value) =>
    onInputChange({ target: { name: 'pickupAddress', value } })
  );
  usePlacesAutocomplete(dropRef, (value) =>
    onInputChange({ target: { name: 'dropAddress', value } })
  );
  ...
  return (
    ...
    >
      <style>{`
        ...
        /* ── Google Places Autocomplete dropdown — dark/light theme ── */
        .pac-container {
          background: ${D ? '#18181b' : '#ffffff'};
          border: 1px solid ${D ? 'rgba(245,158,11,0.25)' : 'rgba(0,0,0,0.1)'};
          border-radius: 12px;
        ...
        .pac-item {
          padding: 10px 14px;
          color: ${D ? '#e4e4e7' : '#18181b'};
          font-size: 12px;
          cursor: pointer;
          border-top: 1px solid ${D ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.06)'};
        }
        ...
        .pac-matched { color: #f59e0b; font-weight: 700; }
      `}</style>
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 text-left">
              {[
                ...
                <div 
                  ...
                  className={`pro-animate-fade px-3 py-1.5 rounded-xl border transition-transform duration-200 hover:scale-[1.02] ${
                    D ? 'bg-zinc-900 border-zinc-800 text-zinc-100' : 'bg-zinc-100 border-zinc-200 text-zinc-900'
                  }`}
                >
        ...
        >
          <div className={`rounded-2xl p-4 sm:p-5 shadow-2xl space-y-3.5 border ${
            ...
            <div 
              ...
              className={`pro-animate-scale p-1 flex rounded-xl border ${
                D ? 'bg-zinc-900/50 border-zinc-800' : 'bg-zinc-100 border-zinc-200'
              }`}
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                ...
                <Field label={t.mobile} dark={D} style={{ animationDelay: '470ms' }} className="pro-animate-scale">
                  ...
                  <input
                    ...
                    value={formData.mobile} onChange={onInputChange}
                    className="bg-transparent w-full text-xs focus:outline-none font-medium text-current"
                  />
                </Field>
                ...
                <Field label={t.date} dark={D} style={{ animationDelay: '620ms' }} className="pro-animate-scale">
                  ...
                  <input
                    ...
                    value={formData.date} onChange={onInputChange}
                    className="bg-transparent w-full text-xs focus:outline-none font-medium text-current"
                  />
                </Field>
                ...
                <Field label={t.time} dark={D} style={{ animationDelay: '670ms' }} className="pro-animate-scale">
                  ...
                  <input
                    ...
                    value={formData.time} onChange={onInputChange}
                    className="bg-transparent w-full text-xs focus:outline-none font-medium text-current"
                  />
                </Field>
                ...
                {tripType === 'roundtrip' && (
                  <>
                    <Field label={t.returnDate} dark={D} style={{ animationDelay: '720ms' }} className="pro-animate-scale">
                      <Calendar className="h-3.5 w-3.5 text-amber-500" />
                      <input
                        type="date" name="returnDate" required
                        value={formData.returnDate} onChange={onInputChange}
                        className="bg-transparent w-full text-xs focus:outline-none font-medium text-current"
                      />
                    </Field>
                    ...
                    <Field label={t.returnTime} dark={D} style={{ animationDelay: '770ms' }} className="pro-animate-scale">
                      <Clock className="h-3.5 w-3.5 text-yellow-500" />
                      <input
                        type="time" name="returnTime" required
                        value={formData.returnTime} onChange={onInputChange}
                        className="bg-transparent w-full text-xs focus:outline-none font-medium text-current"
                      />
                    </Field>
                  </>
